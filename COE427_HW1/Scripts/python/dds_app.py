...
from chat import ChatUser, ChatMessage, FileMessage
import mimetypes

MAX_FILE_NAME_SIZE = 256
MAX_FILE_SIZE = 64000      # 64 KB payload (tune if you want)
...
class Handlers:
    ...
    file_received: Callable[[List[FileMessage]], None] = lambda *_: logging.warning("Not implemented")
...
class DDSApp:
    ...
    TOPIC_NAME_FILE = "file"  # For file messages
    QOS_PROFILE_FILE = "ChatMessage_Profile" 
    ...
    def __init__(self, user : ChatUser, handlers=Handlers(), auto_join=True, domain_id=0):
        ...
        # Create publisher/subscriber for messages *before* using them
        ...
        # DataWriter and DataReader for ChatMessage
        self.writer_msg = dds.DataWriter(
            self.pub_msg,
            self.topic_msg,
            qos=self.qos_provider.datawriter_qos_from_profile(qos_profile_msg_str),
        )

        self.reader_msg = dds.DataReader(
            self.sub_msg,
            self.topic_msg,
            qos=self.qos_provider.datareader_qos_from_profile(qos_profile_msg_str),
        )
        ...
        self.file.fromUser = self.user.username

        qos_profile_file_str = f"{self.QOS_LIBRARY}::{self.QOS_PROFILE_FILE}"

        # This section sets up the FileMessage topic, publisher, subscriber, reader, and monitoring thread
        # ---------- FileMessage entities (for multimedia) ----------
        self.file = FileMessage()
        self.file.fromUser = self.user.username
        ...
        # ------------------------------------------------------------------
        # FileMessage (multimedia)
        # ------------------------------------------------------------------
        self.topic_file = dds.Topic(self.participant, self.TOPIC_NAME_FILE, FileMessage)
        qos_profile_file_str = f"{self.QOS_LIBRARY}::{self.QOS_PROFILE_FILE}"

        # Re-use the same pub/sub created for messages
        self.writer_file = dds.DataWriter(
            self.pub_msg,
            self.topic_file,
            qos=self.qos_provider.datawriter_qos_from_profile(qos_profile_file_str),
        )

        self.reader_file = dds.DataReader(
            self.sub_msg,
            self.topic_file,
            qos=self.qos_provider.datareader_qos_from_profile(qos_profile_file_str),
        )

        self.readcond_file = dds.ReadCondition(self.reader_file, dds.DataState())
        self.waitset_file = dds.WaitSet()
        self.waitset_file.attach_condition(self.stop_condition)
        self.waitset_file.attach_condition(self.readcond_file)

        self.thread_file = threading.Thread(target=self._file_monitor, daemon=True)
    ...
    def file_send(self, destination: str, file_path: str):
        """Send a file (image, video, etc.) as a FileMessage."""

        if not os.path.isfile(file_path):
            logging.error(f"file_send: file does not exist: {file_path}")
            return

        with open(file_path, "rb") as f:
            file_bytes = f.read()

        # truncate if larger than MAX_FILE_SIZE
        if len(file_bytes) > MAX_FILE_SIZE:
            logging.warning(f"file_send: {file_path} too large, truncating to {MAX_FILE_SIZE} bytes")
            file_bytes = file_bytes[:MAX_FILE_SIZE]

        self.file.toUser = destination
        self.file.toGroup = destination
        self.file.fileName = os.path.basename(file_path)
        self.file.mimeType = mimetypes.guess_type(file_path)[0] or "application/octet-stream"
        # Sequence[idl.uint8] expects a list of ints
        self.file.data = list(file_bytes)

        self.writer_file.write(self.file)


    def _file_monitor(self):
        """Monitor the FileMessage topic and invoke file_received on handlers."""

        while True:
            active_conditions = self.waitset_file.wait(dds.Duration(1))

            for cond in active_conditions:
                if cond == self.stop_condition:
                    return

                if cond == self.readcond_file:
                    samples = self.reader_file.take_data()
                    if samples:
                        self.handlers.file_received(samples)
    ...
    def message_send(self, destination, message):
        """Public API: send a text ChatMessage."""

        msg = ChatMessage()
        msg.fromUser = self.user.username

        # In this app we store the destination (either user or group)
        # in both toUser and toGroup; the receiver filters on it.
        msg.toUser = destination
        msg.toGroup = destination

        msg.message = message
        self.writer_msg.write(msg)
